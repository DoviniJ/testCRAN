...
#' This function outputs the summary of regular model and final risk score values of each individual in the target dataset using pre-generated GWAS and/or GWEIS summary statistics, and Polygenic Risk Scores (PRSs) of all the individuals. Note that the input files used in this function can be generated by using GWAS_quantitative and/or GWEIS_quantitative, and PRS_quantitative functions.
...
summary_regular_quantitative <- function(Qphe_target, Qcov_target, trd_score, add_score, gxe_score, Model){
  ...
  if(file.exists(trd_score)){
    prs0_all=read.table(trd_score, header=T)
  ...
  if(file.exists(add_score)){
    prs1_all=read.table(add_score, header=T)
  ...
  if(file.exists(gxe_score)){
    prs2_all=read.table(gxe_score, header=T)
